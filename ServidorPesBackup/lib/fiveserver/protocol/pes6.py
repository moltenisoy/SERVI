    ...
    def backToMatchMenu_4383(self, pkt):
            ...
            for usr in room.participatingPlayers:
              if usr.profile.id == profile.id:
                ...
                if numParticipants == 2:
                    usr.profile.apoints = profile.apoints
                    usr.profile.apoints = profile.apoints
                else:
                    usr.profile.apoints = 0
                    usr.profile.apoints = 0
        ...
        data = b'\0\0\0\0%s%s%s' % (
            b''.join([b'%s%s%s%s%s%s%s%s%s' % (
                ...
                struct.pack('!H',0), # ?
                struct.pack('!H',0), # ?
                struct.pack('!H',usr.profile.points), # new rating
                struct.pack('!H',usr.profile.apoints)) # old rating
                for usr in room.participatingPlayers]),
    ...
    def joinRoom_4320(self, pkt):
        ...
        if room is None:
            ...
            if room.matchSettings:
                data = data + bytes(room.matchSettings.match_time)
